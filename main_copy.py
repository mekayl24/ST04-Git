...
from mathFunctions_copy import process_file, timeToDw, getDw, getInertia, getK, getDragPower, getAppliedPower
...
## Zoomed in graph
plt.plot(TimeStamps[timeInitIndex:timeFinIndex], dt[2:404], marker='o', linestyle='-', color='b', label='Zoomed In')
plt.xlabel('Time (seconds)')
plt.ylabel('Dt')
...
## Zoomed in graph
plt.plot(TimeStamps[timeInitIndex:timeFinIndex], appliedPower[pwrInitIndex:pwrFinIndex], marker='o', linestyle='-', color='b', label='Zoomed In')
plt.xlabel('Time (seconds)')
plt.ylabel('Power (Watts)')
plt.title('Power Data - Zoomed In')
plt.xlim(0, 10)  # Adjust x-axis limits to zoom in on a specific section
plt.ylim(0, 140)  # Adjust y-axis limits if needed
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
